...
CLIPS = [
    ("hero", "Abhishek_01.mp4", "v1", 720, 1280, 0, 9999),
    ("letter_g", "Abhishek_02.mp4", "v2", 540, 960, 0, 9999),
    ("letter_h", "Abhishek_03.mp4", "v3", 540, 960, 0, 9999),
]
...
def build_grunge():
    """Lift the distressed surface out of the supplied ABHISHEK S B artwork.
...
def stage_universe_film():
    ...
    subprocess.run([imageio_ffmpeg.get_ffmpeg_exe(), "-v", "error", "-y", "-i", src,
                    "-c:v", "copy", "-an", "-movflags", "+faststart", dst],
                   check=True)
    subprocess.run([imageio_ffmpeg.get_ffmpeg_exe(), "-v", "error", "-y", "-i", dst,
                    "-frames:v", "1", "-q:v", "4",
