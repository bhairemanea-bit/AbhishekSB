...
export class Clip {
  ...
  whenReady(timeoutMs = 5000) {
    ...
    return new Promise((res) => {
      let timer = null;
      const done = () => { if (timer) clearTimeout(timer); cleanup(); res(this); };
      const cleanup = () => {
      ...
      // Safety valve: if the codec can't stream, don't block the boot sequence forever
      timer = setTimeout(() => { cleanup(); res(this); }, timeoutMs);
    });
  }
