...
export class Finale {
  ...
  async load() {
    const [plate, man, grain] = await Promise.all([
      loadImage('public/Footer_image.png'),
      loadImage('public/tex/grain.png'), // Use safe dummy to prevent GL INVALID_VALUE
  ...
  resize(w, h, dpr) {
    ...
    const s = isPortrait ? Math.min(w / FRAME[0], h / FRAME[1]) : Math.max(w / FRAME[0], h / FRAME[1]);
