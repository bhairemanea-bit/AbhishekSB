...
async function main() {
  ...
  const mk = (name, loopFade) => {
    ...
    if (!c) return null;
    const src = [];
    if (c.webm) src.push({ url: MEDIA + c.webm, type: 'video/webm' });
    if (c.mp4) src.push({ url: MEDIA + c.mp4, type: 'video/mp4' });
    return new Clip({
      src,
  ...
  app.clips.letter_g = mk('letter_g', 0.7);
  app.clips.letter_h = mk('letter_h', 0.7);

  app.heroClips = [app.clips.hero, app.clips.letter_g, app.clips.letter_h].filter(Boolean);
  app.heroIndex = 0;

  // No slider needed: they render side-by-side.
  if (app.furniture.arrowsL) {
    app.furniture.arrowsL.style.opacity = '0';
    app.furniture.arrowsL.style.pointerEvents = 'none';
  }
  if (app.furniture.arrowsR) {
    app.furniture.arrowsR.style.opacity = '0';
    app.furniture.arrowsR.style.pointerEvents = 'none';
  }
  layout();
  window.addEventListener('resize', debounce(layout, 140));
  window.addEventListener('orientationchange', () => setTimeout(layout, 220));

  if (app.heroClips.length > 0) {
    await Promise.all(app.heroClips.map(c => c.whenReady()));
  } else if (app.clips.hero) {
  ...
  let playing = false;
  if (app.heroClips.length > 0) {
    const res = await Promise.all(app.heroClips.map(c => c.play()));
    playing = res.some(Boolean);
  } else if (app.clips.hero) {
  ...
  initUniverse().then((u) => { app.universe = u; })
    .catch((e) => console.warn('[abhishek] universe unavailable:', e.message));
  initChrono().then((c) => { app.chrono = c; })
    .catch((e) => console.warn('[abhishek] chrono unavailable:', e.message));
  initGallery().then((g) => { app.gallery = g; })
    .catch((e) => console.warn('[abhishek] gallery unavailable:', e.message));
  initFinale().then((f) => { app.finale = f; })
    .catch((e) => console.warn('[abhishek] finale unavailable:', e.message));
...
function begin() {
  ...
  if (reduced) {
    ...
    const still = () => {
      if (app.heroClips) {
        app.heroClips.forEach(hc => hc.pause());
      } else {
        c.pause();
      }
      renderStill();
    ...
    if (app.heroClips && app.heroClips.length > 0) {
      app.heroClips.forEach(hc => {
        hc.el.addEventListener('seeked', still, { once: true });
        hc.el.currentTime = Math.min(6, (hc.duration || 8) * 0.6);
      });
    } else {
...
function degrade(reason) {
  console.warn('[abhishek] falling back:', reason);
  root.classList.remove('is-booting');
  root.classList.add('is-fallback');
  boot.classList.add('is-done');
  for (const [, name] of CUES) root.classList.add(`is-${name}`);
  document.querySelector('.stage-wrap').insertAdjacentHTML('afterbegin',
    '<div class="fallback"><p>ABHISHEK</p>'
