...
rectangle) enters centre → the giant red distressed ABHISHEK S B rises from below,
...
top-left dot cluster removed; final layering fixed as background → ABHISHEK S B →
...
Replicate `Footer image.png` exactly: the reference itself is split into
